...
export default function Library() {
  ...
  const visible = useMemo(
    () => sortWorkouts(searchWorkouts(workouts, query), sortBy),
        ...
        {loading ? (
          <>
            ...
            <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
              {Array.from({ length: 3 }).map((_, index) => (
                <div
                  key={index}
                  className="h-72 animate-pulse rounded-xl bg-panel"
                />
              ))}
            </div>
          </>
          ...
          <div className="rounded-xl border border-line bg-panel p-10 text-center">
            ...
            <button
              ...
              className="mt-4 rounded-md bg-accent px-5 py-2 text-sm font-bold text-black"
            >
              Try again
            </button>
          </div>
